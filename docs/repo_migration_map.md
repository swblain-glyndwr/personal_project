...
> **Historical migration record:** this page captures the package-restructure
> decisions and evidence recorded for story `5111778`. It is not current
> operating guidance. Use `docs/repo_structure.md` for the current layout and
> the architecture and CI/CD guides for current runtime and release behaviour.

This document was the control map for moving the NextAds repo into the target production package structure. It is intentionally explicit so later changes can trace the recorded destination, risk level, and validation expectation for each move.
...
next-ads/
  ...
  docs/         # team and engineering context
...
| Documentation | Team, user, and engineering context. |
...
| Retired root `next_ads/` package | Former import bridge and compatibility wrappers. | `src/next_ads/` | Production | Low | Removed after all repository callers moved to supported imports. | Package-root import tests, wheel inspection, unit tests, DAB validation. | Do not recreate the root package; `src/next_ads` is the only package root. |
...
| `sql/` | Production table/view/reporting SQL. | `sql/` | Production | Medium | Stay as the primary SQL folder; add grouping docs. | Table setup tests, DAB validate, table creation smoke where relevant. | Do not move unless grouping is agreed. |
...
| `docs/` | Team docs, workflow docs, release docs, and maintained engineering context. | `docs/` | Documentation | Low | Keep as the documentation home. | Markdown review. | Add migration docs and repository operating context here. |
| `tests/` | Unit and integration tests. | `tests/` | Production | Medium | Keep as the test folder; update imports as modules move. | Tests must continue to run. | Do not move without pytest config update. |
...
| Current path | Current role | Target path | Status | Risk | Move timing | Validation required | Notes |
|---|---|---|---|---|---|---|---|
| `src/next_ads/__init__.py` | Supported package root. | `src/next_ads/__init__.py` | Production | Low | Complete. | Isolated `PYTHONPATH=src` import test and wheel inspection. | The former root-package bridge has been removed. |
...
| `src/next_ads/decisioning/assignment.py` | Assignment, greedy allocation, preranked ads, NextGenAds, and algorithm-division logic. | Same | Production | High | Complete. | Assignment unit tests and representative output comparison before behavioural edits. | Supported replacement for the removed `Assignment` wrapper. |
| `src/next_ads/control/attributes.py` | Attribute parsing and theme/control helpers. | Same | Production | Medium | Complete. | Attribute tests. | Supported replacement for the removed `Attributes` wrapper. |
| `src/next_ads/ranking/scoring.py` | Model score retrieval and aggregation. | Same | Production | High | Complete. | Scoring and ranking tests. | Supported replacement for the removed `Scoring` wrapper. |
| `src/next_ads/reporting/results.py` | Result aggregation, checks, and reporting helpers. | Same | Production | Medium | Complete. | Reporting tests and output sanity checks. | Supported replacement for the removed `Results` wrapper. |
| `src/next_ads/reporting/plotting.py` | Graph plotting helpers. | Same | Operational-transition | Low | Complete. | Import test. | Supported replacement for the removed `Plotting` wrapper. |
...
| `src/next_ads/common/config_manager.py` | Dynaconf configuration loading and environment resolution. | Same | Production | Medium | Complete. | Config tests for dev/preprod/prod and supported clients. | All repository callers use the supported import. |
| `src/next_ads/common/etl.py` | Shared ETL/table-name helpers. | Same | Production | Medium | Complete. | ETL behaviour tests. | All repository callers use the supported import. |
| `src/next_ads/delivery/google_sheets.py` | Google Sheets and PLP delivery helpers. | Same | Production | Medium | Complete. | PLP GS tests and smoke. | Primary delivery implementation. |
...
| `jobs/nextads_candidates/build_theme_scores.py` | `build_markov_scores` shadow-provider task. | `jobs/nextads_candidates/build_theme_scores.py` | Production | High | Moved into the independent Markov scoring job and adapted to the shared score-source contract. | Job-path tests, DAB validate, DEV Integration run before release; shared/legacy score parity before behavioural edits. | Ranking/scoring-affecting. It stages one exact score build; the shared publisher validates it and derives the legacy output without gating the candidate route. |
...
| Current path | Current role | Target path | Status | Risk | Move timing | Validation required | Notes |
|---|---|---|---|---|---|---|---|
| External Databricks `Hackathon_job` definition | Historical scheduled Theme Affinity workflow that ran `hackathon_model` notebooks and wrote legacy hackathon-named outputs. | Initially `pipelines/databricks/jobs/mktg_next_uk_nextads_theme_affinity.yml`; now consolidated into the parameterised `pipelines/databricks/jobs/mktg_next_uk_nextads_model_scoring.yml` route. | Operational-transition | High | Bring the definition into the repository before moving notebooks or changing model code, then consolidate stable scoring behind the generic model-scoring contract. | DAB validate, task/parameter contract checks and Databricks run evidence. | Preserve legacy output contracts where consumers still require them, but do not add another model-specific saved scoring job. |
...
| `docs/docs_for_ai/` | Maintained repository context for contributors. | Keep the existing path or rename it only if the team agrees a clearer descriptive name. | Documentation | Low | Keep or rename only through a docs-focused PR. | Markdown review and link check. | This should explain repo structure, release route, domain concepts, safe-edit boundaries, output contracts, and known gotchas so contributors do not infer unsafe changes. |
| Future model/domain context docs | Compact context files for operational models such as Theme Affinity, pCTR, Ads v2, realtime, and assignment/decisioning. | Keep alongside the human-readable domain docs. | Documentation | Low | Add as model/domain areas are migrated. | Markdown review by domain owner. | These files should state ownership, current and target paths, production outputs, allowed edits, validation commands, and release evidence expectations. |
...
- Maintained repository context is part of the target structure. It should sit alongside the main documentation so contributors can follow the repo structure, release route, and production safety boundaries without guessing.
