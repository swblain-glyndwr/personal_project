...
| `jobs/nextads_control/parse_theme_mapping.py` | Route-aware entrypoint for `parse_theme_mapping_v1` and `parse_theme_mapping_v2`. | `jobs/nextads_control/parse_theme_mapping.py` | Production | Medium | Now accepts `--route` so v1 and v2 write separate theme mapping/item-theme outputs. | Job-path tests, DAB validate, theme mapping table sanity check before release. | Control metadata. The main DAB job now points both route tasks at `jobs/nextads_control/parse_theme_mapping.py`. |
| `jobs/nextads_control/compare_theme_mappings.py` | Compares v1/v2 Theme Mapping tabs and warns on differences before v2 parsing. | `jobs/nextads_control/compare_theme_mappings.py` | Operational-transition | Medium | Added for Trade call-out during v1/v2 parallel running. | Unit diff test, DEV Integration run, warning check. | Control metadata; does not write route tables. |
| `jobs/nextads_candidates/build_theme_scores.py` | Route-aware entrypoint for `score_lightweight_v1` and `score_lightweight_v2`. | `jobs/nextads_candidates/build_theme_scores.py` | Production | High | Now accepts `--route` so v2 can score from `item_themes_latest_v2`. | Job-path tests, DAB validate, DEV Integration run before release; score comparison before behavioural edits. | Ranking/scoring-affecting. The main DAB job points both lightweight route tasks at this script. |
...
| `jobs/nextads_candidates/build_page_type_candidates_v2.py` | `map_theme_scores_to_ads_v2` task. Reads the v2 control sheet and v2 lightweight theme scores directly, then calls the shared ranking mapper at page-type grain. | `jobs/nextads_candidates/build_page_type_candidates_v2.py`; reusable ranking/mapping logic remains in `src/next_ads/ranking/theme_score_mapping.py` until a v2-specific package split is justified. | Operational-transition | High | Active parallel route. | Output checks, ranking comparison, DEV Integration run, PREPROD validation if retained. | Reads `control_sheet_latest_v2` and `next_theme_scores_latest_v2`, writes `preranked_ads_from_themes_v2_latest`, and does not depend on `preranked_ads_from_themes_latest`. |
