...
| `next_ads/Attributes.py` | Attribute parsing and theme/control attribute helpers. | `src/next_ads/control/attributes.py` | Production | Medium | Before assignment move, after tests cover parsing. | Attribute unit tests, control sheet smoke if used. | Affects input/control interpretation. |
...
| `next_ads/utils/etl.py` | Shared ETL/table helpers. | `src/next_ads/common/etl.py` | Production | High | After table operation tests exist. | Unit tests plus table-write route review. | Write helpers can affect outputs. |
