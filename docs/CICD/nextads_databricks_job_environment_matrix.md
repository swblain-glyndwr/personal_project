...
| `DEV` | Developer-specific feature-branch validation in the DEV workspace. | Operational routes plus personal Feature Store, model-development, model-research, smoke and setup jobs. |
...
| Personal DEV model research | `mktg_next_uk_nextads_model_research`<br>`mktg_next_uk_nextads_model_research_selection`<br>`mktg_next_uk_nextads_model_research_automl` | `DEV` | Manual research, reviewed-selection and discovery routes. AutoML is disabled unless explicitly enabled. None of these jobs selects a serving provider or changes assignments. |
