...
| --- | --- | --- | --- |
| Operational inputs and scoring | `mktg_next_uk_nextads_scoring_inputs`<br>`mktg_next_uk_nextads_model_scoring`<br>`mktg_next_uk_nextads_markov_scoring`<br>`mktg_next_uk_nextads_candidate_foundation`<br>`mktg_next_uk_nextads_candidate_build`<br>`mktg_next_uk_nextads_data_pull` | `SANDBOX`, `DEV`, `DEV_INTEGRATION`, `PREPROD`, `PROD` | The unscheduled scoring-inputs job prepares the reusable accepted input snapshot. The generic scoring job currently supports `model_name=theme_affinity` and calls that job for the same date before scoring. Candidate Build remains an independent 18:00 advert-option job. Pipeline-backed jobs travel with their referenced pipeline resources. |
