...
The target shape is model-neutral. The feature store is for Next Ads-wide reusable batch features, not a pCTR-specific store. pCTR, LTR/theme affinity, two-tower retrieval, and future ranking models should consume these governed feature tables instead of rebuilding source-specific feature joins inline.
...
| Response-model/pCTR product embeddings | Item/product embeddings, advert product vectors, customer/ad product similarity inputs | Item/product or advert, feature date, embedding model/version | `next_uk_nextads_fs_product_embeddings_latest`, `next_uk_nextads_fs_advert_product_profile_daily` |
...
| 2 | `next_uk_nextads_fs_account_web_activity_90d` | Reusable customer activity features needed by pCTR, LTR and retrieval models. |
