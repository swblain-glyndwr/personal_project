...
| 2 | Populate Theme Affinity feature-store slice | Theme affinity feature tables, labels and model input | First populated slice; reads stable production Theme Affinity runtime tables and leaves operational outputs unchanged. |
| 3 | Validate Theme Affinity compatibility view | `next_uk_nextads_theme_affinity_features_latest` | Preserve current `experiments/hackathon_theme_affinity_model/config.py` feature list while proving output equivalence. |
