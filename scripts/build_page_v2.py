...
df_ad_assigned = (
    df_ad_assigned
    .join(df_ad_treatments, on=['AccountNumber', 'Rank'], how='left')
    .withColumn(
        'Treatment',
        F.when(
            ((F.col('IsPremium') == 1)
...
n_null_treatment = (
    df_ad_assigned.where(F.col("Treatment").isNull()).count()
    )
...
if n_null_measure > 0:
    null_measure_msg = (f"{n_null_measure:,} assignments removed during " +
...
delete_from_and_load(df_ad_assigned,
