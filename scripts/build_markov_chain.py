...
if TRAIN:
    ...
    transition_probs = (
        ...
        .withColumn(
            "transition_freq", F.col("transition_freq").cast("decimal(12,2)")
        )
        .withColumn("theme_total", F.col("theme_total").cast("integer"))
        .withColumn("probability", F.col("probability").cast("decimal(10,9)"))
        .withColumn(
            "base_probability", F.col("base_probability").cast("decimal(10,9)")
        )
        .withColumn(
            "probability_rebased",
...
if VIEWS_ENABLED and account_view_themes is not None:
    ...
    combined = (
        ...
        .na.fill(0)
        .withColumn(
            "prob_agg", F.col("score_buy") + (F.col("score_view") * F.lit(0.1))
        )
    )
else:
