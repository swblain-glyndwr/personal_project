...
def validate_required_column_values(
    df: Any,
    required_columns: list[str] | tuple[str, ...],
    table_name: str,
) -> None:
    """Fail fast when a feature table has null primary-key values."""
    ...
    if not required_columns:
        return
    null_condition = reduce(or_, (F.col(column).isNull() for column in required_columns))
    has_null_keys = df.where(null_condition).limit(1).count() > 0
    if has_null_keys:
        raise ValueError(
            f"{table_name} contains null values for required feature-store "
            f"key columns: {', '.join(required_columns)}"
        )
