with base as (
  SELECT
    ...
    max(if(order_date between reference_date - 28 and "{end_date_baskets}", 1, 0)) as recency28,
    max(if(order_date between reference_date - 7 and "{end_date_baskets}", 1, 0)) as recency7
  FROM {schema}.{table_prefix}_baskets_themes
