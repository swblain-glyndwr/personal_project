with atbs as (
  SELECT DISTINCT
    ...
    timestamp,
    reference_date
  FROM {schema}.{table_prefix}_atbs_themes
...
base as (
  SELECT
    ...
    max(if(atbs.date between reference_date -28 and "{end_date_atbs}", 1, 0)) as recency28,
    max(if(atbs.date between reference_date - 7 and "{end_date_atbs}", 1,0)) as recency7
  FROM atbs
  group by ALL
)
SELECT distinct
