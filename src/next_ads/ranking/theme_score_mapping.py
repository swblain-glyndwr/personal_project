...
def run_theme_score_mapping(
    *,
    ...
    logger.info("Finding distinct ad sets across locations to minimise repeated ranking")
    df_ad2loc, df_adset2loc, df_ad2adset = build_ad_location_mappings(
        spark,
    ...
    df_adset_scores = rank_top_ads_per_adset(
        ...
        customer_prefs,
        age_order_map,
        top_ads_per_location,
    )
    df_adset_scores.cache()

    logger.info("Mapping ranked ads back to locations")
    df_ad_scores = map_ranked_ads_to_locations(df_adset_scores, df_adset2loc)

    logger.info("Checking for ads assigned to ineligible locations")
    assert_eligible_locations(df_ad_scores, df_ad2loc)
