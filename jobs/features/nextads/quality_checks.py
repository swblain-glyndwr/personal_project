...
def _quality_counts(dataframe, primary_keys: tuple[str, ...]) -> tuple[int, int, int]:
    null_key_condition = _null_key_condition(primary_keys)
    counts = dataframe.agg(
        F.count("*").alias("row_count"),
        F.countDistinct(*primary_keys).alias("distinct_key_count"),
...
def main() -> None:
    ...
    for table_name in FEATURE_STORE_QUALITY_TABLES:
        ...
        row_count, distinct_key_count, null_key_count = _quality_counts(
            dataframe,
            table.primary_keys,
        )
