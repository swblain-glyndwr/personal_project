...
logger.info(
    f"  Weight at end of window:"
    f" {math.exp(-DECAY_LAMBDA * TIME_WINDOW_DAYS):.1%}"
)

items_recent = spark.table(BASKETS).filter(
...
baskets_cum_pct = baskets_grouped.withColumn(
    "cum_spend", F.sum("total_spend").over(window_spec)
).withColumn("cum_pct_spend", F.col("cum_spend") / F.lit(total_sum))

items_filtered = baskets_cum_pct.filter(
    F.col("cum_pct_spend") <= REVENUE_PERCENTILE
).select("itemno")
...
rpid_lookup = (
    ...
    .unionByName(
        spark.table(BQ_SESSIONS_APP).select(
            "UniqueVisitID", "AccountNumber_RPID", "date"
        ),
    ...
    .select(
        "UniqueVisitID", F.col("AccountNumber_RPID").alias("account_number")
    )
    .distinct()
)
...
views = (
    ...
    .agg(F.min("timestamp").alias("timestamp"), F.min("date").alias("date"))
...
purchases = (
    ...
    .filter(
        ...
        & (
            F.col("order_date")
            > F.date_sub(F.current_date(), TIME_WINDOW_DAYS)
        )
    )
    .select(
        "account_number",
...
pairs_raw = (
    views.alias("t1")
    .join(purchases.alias("t2"), on=["account_number"], how="inner")
    .filter(
        (F.col("t2.date") >= F.col("t1.date"))
        & (
            F.col("t2.date")
            <= F.date_add(F.col("t1.date"), PURCHASE_WINDOW_DAYS)
        )
...
views_count = pairs_raw.groupBy("itemno1").agg(
    F.countDistinct("account_number").alias("views")
)

purchases_count = pairs_raw.groupBy("itemno2").agg(
    F.countDistinct("account_number").alias("purchases")
)

pair_count = pairs_raw.groupBy("itemno1", "itemno2").agg(
    F.countDistinct("account_number").alias("freq12"),
    F.sum(
        F.exp(-DECAY_LAMBDA * F.datediff(F.current_date(), F.col("view_date")))
    ).alias("freq12_decay"),
)
...
stats_raw = (
    pair_count.alias("t0")
    .join(
        views_count.alias("t1"),
        F.col("t0.itemno1") == F.col("t1.itemno1"),
        "left",
    )
    .join(
        purchases_count.alias("t2"),
        F.col("t0.itemno2") == F.col("t2.itemno2"),
        "left",
    )
    .select(
        F.col("t0.itemno1"),
...
stats_final = (
    stats_raw.withColumn("all_customers", F.lit(total_customers))
    ...
    .withColumn(
        "lift", F.col("support12") / (F.col("support1") * F.col("support2"))
    )
    .withColumn(
        "lift_adjusted", F.col("lift") * F.pow(F.col("support2"), 0.25)
    )
    .withColumn(
        "cosine_similarity",
