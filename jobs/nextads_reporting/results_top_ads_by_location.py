...
from dsutils.etl import truncate_and_load, post_to_webhook
...
exclude_locations_like = cfg["real_time_unknown"]["backfill"][
    "exclude_locations_like"
]
exclude_pattern = r"^(%s)" % "|".join(map(re.escape, exclude_locations_like))
...
df_ad_performance_by_loc = (
    ...
    .withColumn(
        "IncARPSAdj", F.col("IncARPS") / F.col("AvgSessionOverlapRatio")
    )
...
window_spec = Window.partitionBy("Location").orderBy(
    F.col("EstContribution").desc(), F.col("UniqueAdID").asc()
)

df_with_rank = df_ad_performance_by_loc.withColumn(
    "rank", F.row_number().over(window_spec)
)

df_top_ad_by_loc = df_with_rank.filter(F.col("rank") == 1).select(
    "Location", "UniqueAdID", "MASIDToken"
)
...
if df_top_ad_by_loc.isEmpty():
    ...
    truncate_and_load(df_top_ad_by_loc, TOP_ADS_BY_LOC, pk_cols=["Location"])
