...
from dsutils.etl import assert_pk, delete_from_and_load, post_to_webhook
...
RESULTS_AGGREGATED_TABLE = etl.map_tbl(
    tbls["results_aggregated"], **tbl_args_op
)
RESULTS_AB_TABLE = etl.map_tbl(tbls["results_ab"], **tbl_args_op)
RESULTS_ADS_TABLE = etl.map_tbl(tbls["results_ads"], **tbl_args_op)
RESULTS_ADS_LOCATION_TABLE = etl.map_tbl(
    tbls["results_ads_location"], **tbl_args_op
)
...
RESULTS_ADS_TARGETING_TABLE = etl.map_tbl(
    tbls["results_ads_targeting"], **tbl_args_op
)
RESULTS_PAGE_TARGETING_TABLE = etl.map_tbl(
    tbls["results_page_targeting"], **tbl_args_op
)
RESULTS_AD_METADATA_TABLE = etl.map_tbl(
    tbls["results_ad_metadata"], **tbl_args_op
)
...
df_sessions_master_meta = spark.read.format("parquet").load(
...
process_dates = [
    x[0]
    for x in df_sessions_master_meta.select("SessionDate").distinct().collect()
]

logger.info("Reading: df_ad_metadata_non_loc")
df_ad_metadata_non_loc = spark.read.format("parquet").load(
...
df_ad_metadata = spark.read.format("parquet").load(
    f"{TMP_RESULTS_LOCATION}/df_ad_metadata"
)
df_ad_metadata = df_ad_metadata.drop("Page", "Screen", "PageGroup")
df_ad_metadata.cache()

logger.info("Reading: df_fixed_cells")
df_fixed_cells = spark.read.format("parquet").load(
...
df_summary_device_os_wide = (
    df_summary_device_os.where(
        F.col("FallowControl").isin(FALLOW_FALSE, FALLOW_TRUE)
    )
...
for c in df_summary_device_os_wide.columns:
    df_summary_device_os_wide = df_summary_device_os_wide.withColumnRenamed(
        c, c.replace(f"{FALLOW_TRUE}_", "C_").replace(f"{FALLOW_FALSE}_", "")
    )

df_summary_device_os_wide.cache()

total_r = df_summary_device_os_wide.agg(F.sum("Revenue")).collect()[0][0]
total_apr = df_summary_device_os_wide.agg(
    F.sum("ApportionedRevenue")
).collect()[0][0]
...
for ac in agg_cols:
    ...
    df_summary_ac_renamed = (
        df_summary_ac.withColumnRenamed(ac, "AggValue")
...
all_tags = (
    df_ad_metadata_non_loc.withColumn("TagsSplit", F.split("Tags", ", "))
    .select("TagsSplit")
    .distinct()
...
if agg_tags:
    for agg_tag in agg_tags:
        df_summary_agg_tag = summarise_sessions(
            (
                df_sessions_master_meta.withColumn(
                    "TagArr", F.split("Tags", ", ")
                )
                .withColumn(
                    "Tagged",
                    F.when(
                        F.array_contains(F.col("TagArr"), agg_tag), 1
                    ).otherwise(0),
                )
        ...
        df_summary_agg_tag_renamed = df_summary_agg_tag.withColumn(
            "AggValue", F.lit(agg_tag)
...
for c in df_summary_agg_wide.columns:
    df_summary_agg_wide = df_summary_agg_wide.withColumnRenamed(
        c, c.replace(f"{FALLOW_TRUE}_", "C_").replace(f"{FALLOW_FALSE}_", "")
    )

df_summary_agg_wide = append_session_overlap_ratio(
    df_summary_device_os_wide,
    df_summary_agg_wide,
    SESSION_LEVEL_COLS,
...
for agg_col in agg_cols:
    total_apr_agg = (
        df_summary_agg_wide.where(F.col("AggColumn") == agg_col)
        .agg(F.sum("ApportionedRevenue"))
        .collect()[0][0]
    )
    ...
    if diff_agg < -0.01 * total_r:
        msg_warn = (
            f"Total of ApportionedRevenue (agg: {agg_col}) more than"
...
df_summary_ad = (
    summarise_sessions(
        ...
        **col_args_dict,
        group_cols=(
            SESSION_LEVEL_COLS + ["FallowControl", "UniqueAdIDMeasurement"]
        ),
    )
...
df_summary_ad_wide = (
    df_summary_ad.where(F.col("FallowControl").isin(FALLOW_FALSE, FALLOW_TRUE))
...
for c in df_summary_ad_wide.columns:
    df_summary_ad_wide = df_summary_ad_wide.withColumnRenamed(
        c, c.replace(f"{FALLOW_TRUE}_", "C_").replace(f"{FALLOW_FALSE}_", "")
    )

df_summary_ad_wide = append_session_overlap_ratio(
    df_summary_device_os_wide, df_summary_ad_wide, SESSION_LEVEL_COLS
)

df_summary_ad_wide.cache()

total_apr_ad = df_summary_ad_wide.agg(F.sum("ApportionedRevenue")).collect()[
    0
][0]
...
if diff_ad < -0.01 * total_r:
    msg_warn = (
        "Total of ApportionedRevenue (ads) more than 1% "
...
df_summary_ad_locset = (
    summarise_sessions(
        (
            df_sessions_master_meta.withColumn(
                "LocationSet", F.collect_set("Location").over(w_visit_ad)
            )
        ),
        **col_args_dict,
        group_cols=(
    ...
    .withColumnRenamed("UniqueAdIDMeasurement", "UniqueAdID")
    .withColumn(
        "LocationSet", F.concat_ws("+", (F.array_sort(F.col("LocationSet"))))
    )
)

df_summary_ad_locset_wide = (
    df_summary_ad_locset.where(
        F.col("FallowControl").isin(FALLOW_FALSE, FALLOW_TRUE)
    )
...
for c in df_summary_ad_locset_wide.columns:
    df_summary_ad_locset_wide = df_summary_ad_locset_wide.withColumnRenamed(
        c, c.replace(f"{FALLOW_TRUE}_", "C_").replace(f"{FALLOW_FALSE}_", "")
    )

df_summary_ad_locset_wide = append_session_overlap_ratio(
    df_summary_device_os_wide, df_summary_ad_locset_wide, SESSION_LEVEL_COLS
)
...
df_summary_ad_pagegroupset = (
    summarise_sessions(
        (
            df_sessions_master_meta.withColumn(
                "PageGroupSet", F.collect_set("PageGroup").over(w_visit_ad)
            )
        ),
        **col_args_dict,
        group_cols=(
    ...
    .withColumnRenamed("UniqueAdIDMeasurement", "UniqueAdID")
    .withColumn(
        "PageGroupSet", F.concat_ws("+", (F.array_sort(F.col("PageGroupSet"))))
    )
)

df_summary_ad_pagegroupset_wide = (
    df_summary_ad_pagegroupset.where(
        F.col("FallowControl").isin(FALLOW_FALSE, FALLOW_TRUE)
    )
...
for c in df_summary_ad_pagegroupset_wide.columns:
    df_summary_ad_pagegroupset_wide = (
        df_summary_ad_pagegroupset_wide.withColumnRenamed(
            c,
            c.replace(f"{FALLOW_TRUE}_", "C_").replace(f"{FALLOW_FALSE}_", ""),
        )
    )

df_summary_ad_pagegroupset_wide = append_session_overlap_ratio(
    df_summary_device_os_wide,
    df_summary_ad_pagegroupset_wide,
    SESSION_LEVEL_COLS,
)
...
df_summary_div_pagegroupset = (
    summarise_sessions(
        (
            df_sessions_master_meta.withColumn(
                "PageGroupSet", F.collect_set("PageGroup").over(w_visit_div)
            )
        ),
        **col_args_dict,
        group_cols=(
...
df_summary_div_pagegroupset_wide = (
    df_summary_div_pagegroupset.where(
        F.col("FallowControl").isin(FALLOW_FALSE, FALLOW_TRUE)
    )
...
for c in df_summary_div_pagegroupset_wide.columns:
    df_summary_div_pagegroupset_wide = (
        df_summary_div_pagegroupset_wide.withColumnRenamed(
            c,
            c.replace(f"{FALLOW_TRUE}_", "C_").replace(f"{FALLOW_FALSE}_", ""),
        )
    )

df_summary_div_pagegroupset_wide = append_session_overlap_ratio(
    df_summary_device_os_wide,
    df_summary_div_pagegroupset_wide,
    SESSION_LEVEL_COLS,
)
...
df_summary_tdiv_pagegroupset = (
    summarise_sessions(
        (
            df_sessions_master_meta.withColumn(
                "PageGroupSet", F.collect_set("PageGroup").over(w_visit_tdiv)
            )
        ),
        **col_args_dict,
        group_cols=(
...
df_summary_tdiv_pagegroupset_wide = (
    df_summary_tdiv_pagegroupset.where(
        F.col("FallowControl").isin(FALLOW_FALSE, FALLOW_TRUE)
    )
...
for c in df_summary_tdiv_pagegroupset_wide.columns:
    df_summary_tdiv_pagegroupset_wide = (
        df_summary_tdiv_pagegroupset_wide.withColumnRenamed(
            c,
            c.replace(f"{FALLOW_TRUE}_", "C_").replace(f"{FALLOW_FALSE}_", ""),
        )
    )

df_summary_tdiv_pagegroupset_wide = append_session_overlap_ratio(
    df_summary_device_os_wide,
    df_summary_tdiv_pagegroupset_wide,
    SESSION_LEVEL_COLS,
)
...
df_ad_elig_locs = (
    df_ad_metadata.withColumn(
        "EligibleLocations", F.collect_set(F.col("Location")).over(w_date_ad)
    )
...
df_ad_served_locs = (
    df_sessions_master_meta.where(F.col("UniqueAdIDMeasurement").isNotNull())
    .withColumnRenamed("UniqueAdIDMeasurement", "UniqueAdID")
    .withColumn(
        "ServedLocations", F.collect_set(F.col("Location")).over(w_date_ad)
    )
...
df_ad_metadata_full = df_ad_metadata_non_loc.join(
    df_ad_elig_locs, on=["SessionDate", "UniqueAdID"], how="left"
).join(df_ad_served_locs, on=["SessionDate", "UniqueAdID"], how="left")
...
for d in process_dates:
    ...
    logger.info(
        f"Loading results_topline for {d_fmt} "
        + f"to table: {RESULTS_TOPLINE_TABLE}"
    )
    delete_from_and_load(
        (
            df_summary_device_os_wide.where(F.col("SessionDate") == d).select(
                "SessionDate",
                "Device",
                "OS",
                "Sessions",
    ...
    logger.info(
        f"Loading results_aggregated for {d_fmt} "
        + f"to table: {RESULTS_AGGREGATED_TABLE}"
    )
    delete_from_and_load(
        (
            df_summary_agg_wide.where(F.col("SessionDate") == d).select(
                "SessionDate",
                "Device",
                "OS",
                "AggColumn",
        ...
        pk_cols=["SessionDate", "Device", "OS", "AggColumn", "AggValue"],
        del_where={"SessionDate": d_fmt},
    )
    ...
    logger.info(
        f"Loading results_ads for {d_fmt} " + f"to table: {RESULTS_ADS_TABLE}"
    )
    delete_from_and_load(
        (
            df_summary_ad_wide.where(F.col("SessionDate") == d).select(
                "SessionDate",
                "Device",
                "OS",
                "UniqueAdID",
    ...
    logger.info(
        f"Loading results_ads_location for {d_fmt} "
        + f"to table: {RESULTS_ADS_LOCATION_TABLE}"
    )
    delete_from_and_load(
        (
            df_summary_ad_locset_wide.where(F.col("SessionDate") == d).select(
                "SessionDate",
                "Device",
                "OS",
                "UniqueAdID",
        ...
        pk_cols=["SessionDate", "Device", "OS", "UniqueAdID", "LocationSet"],
        del_where={"SessionDate": d_fmt},
    )

    logger.info(
        f"Loading results_ads_page for {d_fmt} "
        + f"to table: {RESULTS_ADS_PAGE_TABLE}"
    )
    delete_from_and_load(
        (
            df_summary_ad_pagegroupset_wide.where(
                F.col("SessionDate") == d
            ).select(
                "SessionDate",
                "Device",
        ...
        pk_cols=["SessionDate", "Device", "OS", "UniqueAdID", "PageGroupSet"],
        del_where={"SessionDate": d_fmt},
    )

    logger.info(
        f"Loading results_div_page for {d_fmt} "
        + f"to table: {RESULTS_DIV_PAGE_TABLE}"
    )
    delete_from_and_load(
        (
            df_summary_div_pagegroupset_wide.where(
                F.col("SessionDate") == d
            ).select(
                "SessionDate",
                "Device",
    ...
    logger.info(
        f"Loading results_tdiv_page for {d_fmt} "
        + f"to table: {RESULTS_TDIV_PAGE_TABLE}"
    )
    delete_from_and_load(
        (
            df_summary_tdiv_pagegroupset_wide.where(
                F.col("SessionDate") == d
            ).select(
                "SessionDate",
                "Device",
    ...
    logger.info(
        f"Loading results_ad_metadata for {d_fmt} "
        + f"to table: {RESULTS_AD_METADATA_TABLE}"
    )
    delete_from_and_load(
        (
            df_ad_metadata_full.where(F.col("SessionDate") == d).select(
                "SessionDate",
                "UniqueAdID",
                "PotNumber",
                "CampaignNumber",
