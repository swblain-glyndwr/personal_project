...
df_rt = (
    spark.table("marketingdata_prod.warehouse.rtp_exponea_tracking")
    .withColumn(
        "parsed_body", F.from_json(F.col("request_body"), request_body_schema)
    )
    .select(
        F.col("ID"),
        F.col("parsed_body.customer_ids")
        .getItem("anon_rpid")
        .alias("AnonRPID"),
        F.col("parsed_body.properties").getItem("rt_masid").alias("MASID"),
        F.col("parsed_body.update_timestamp").alias("UpdateTimestampUnix"),
        F.from_unixtime(
            F.col("parsed_body.update_timestamp"), "yyyy-MM-dd HH:mm:ss"
        )
        .cast(TimestampType())
...
df_latest_actions = (
    df_actions.join(
        df_rt_distinct, df_actions.RPID == df_rt_distinct.AnonRPID, "inner"
    )
...
df_purchases = df_latest_actions.join(
    df_transactions, "UniqueVisitID", "inner"
).select(
    "UniqueVisitID",
    "AnonRPID",
    "TreatmentGroup",
...
logger.info(
    f"Num distinct RPIDs with purchases: {df_purchases.select('RPID').distinct().count()}"
)  # noqa

df_metrics_agg = (
    df_latest_actions.join(
        df_transactions.select(
            "UniqueVisitID", "TransactionID", "ProductRevenue"
        ),
        on="UniqueVisitID",
        how="left",
    )
    ...
    .agg(
        ...
        F.sum(F.coalesce(F.col("ProductRevenue"), F.lit(0))).alias(
            "total_revenue"
        ),
    )
)
...
df_metrics = (
    df_treatment_groups.join(df_metrics_agg, on="TreatmentGroup", how="left")
    .withColumn(
    ...
    .withColumn(
        "RPV", F.round(F.col("total_revenue") / F.col("total_sessions"), 4)
    )
    .withColumn(
        "CVR", F.round(F.col("total_orders") / F.col("total_sessions"), 4)
    )
    .withColumn(
        "AOV",
        F.round(
    ...
    .select(F.lit(session_date_str).cast("date").alias("SessionDate"), "*")
)
df_metrics.show()
