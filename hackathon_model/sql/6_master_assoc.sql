WITH
  ...
  baskets_target AS (
    ...
    FROM {catalog}.{table_prefix}_baskets_target
    WHERE reference_date = date"{reference_date}"
  ),
  repurchased AS (
  ...
  advanced_features AS (
    SELECT *
    FROM {catalog}.{table_prefix}_advanced_features
    WHERE reference_date = date"{reference_date}"
  ),
  customer_features AS (
    SELECT *
    FROM {catalog}.{table_prefix}_customer_features
    -- WHERE reference_date = date"{reference_date}"
  ),
  ...
  popularity_metrics AS (
    SELECT *
    FROM {catalog}.{table_prefix}_popularity_metrics
    -- WHERE reference_date = date"{reference_date}"
  )
...
SELECT
  ...
  "{start_date_view} > {end_date_view}" as views_dates,
  "{start_date_atbs} > {end_date_atbs}" as atbs_dates,
  "{start_date_bask} > {end_date_bask}" as baskets_dates,
  "{start_date_target} > {end_date_target}" as target_dates,
