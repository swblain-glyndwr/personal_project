SELECT
  ...
  T1.account_number,
  T2.theme_clean2,
  sum(freq12) as theme_clean2_baskets_freq12,
  sum(lift) as theme_clean2_baskets_lift,
  max(CASE WHEN freq12 >= 5 THEN lift else 0 end) as theme_clean2_baskets_lift_max,
  sum(cs) as theme_clean2_baskets_cs,
  array_agg(distinct theme_clean1) as seed_theme, 
  current_date() as rundate
FROM {catalog}.{table_prefix}_baskets_bytheme T1
