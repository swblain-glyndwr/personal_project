...
class TestLoadControlSheetConfigParityHighLevel:
    ...
    def test_locations_map_structure_exists(self, json_config, dynaconf_config):
        for location_key in json_config["locations"].keys():
            json_location = json_config["locations"][location_key]
            dynaconf_location = dynaconf_config.locations[location_key]

            if "map" in json_location:
                assert hasattr(dynaconf_location, "map"), (
                    f"Location {location_key} missing map"
                )
                assert len(json_location["map"]) == len(dynaconf_location.map), (
                    f"Location {location_key} map length mismatch"
                )
    ...
    def test_tables_write_config_matches(self, json_config, dynaconf_config):
        ...
        for table in tables_to_check:
            ...
            json_table = etl.map_tbl(json_config["tables"]["write"][table], **tbl_args)
            assert (
                json_table
