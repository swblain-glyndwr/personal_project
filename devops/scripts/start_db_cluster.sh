...
if ! databricks clusters get "$CLUSTER_ID" &> /dev/null; then
    echo "Error: Cluster $CLUSTER_ID does not exist"
    echo "Available clusters:"
    databricks clusters list --output json || true
    exit 1
fi

# Get current cluster status
CLUSTER_STATE=$(databricks clusters get "$CLUSTER_ID" | jq -r '.state')
...
while [ $ELAPSED -lt $MAX_WAIT_TIME ]; do
    CURRENT_STATE=$(databricks clusters get "$CLUSTER_ID" | jq -r '.state')
